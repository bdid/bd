...
require_once (realpath(dirname(__FILE__) . '/Database/connectionQa.php'));
...
<div id="main_container" class="main_container">
	<header class="header">
		...
		<div class='right'>
			<span id="filter" >Filter</span><span class="arrow">></span>
	...
	<div class="filter_div shadow">
		<div class="checkboxDiv">
			<table>
				<tr>
					<td><div class="checkbox inactive"></div><span>abcdef</span></td>
					<td><div class="checkbox inactive"></div><span>abcdef</span></td>
					<td rowspan="2">
...
function getDocInfo($docInfoJson) {
	...
	while ($row = mysql_fetch_assoc($docInfoJson)) {
		...
		$retStr .= '<h3>' . $row["f_name"] . ' ' . $row["l_name"] . ', ' . $row["degree"] . '</h3>';
		$retStr .= '<h6>' . $row["f_name"] . '</h6>';
		$retStr .= '<h6>' . $row["hospital"] . ', ' . $row["country"] . '</h6>';
